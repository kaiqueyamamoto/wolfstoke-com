export default function SolucoesHeader() {
  return (
    <header className="sticky top-0 z-20 bg-black backdrop-blur">
      <div className="navbar mx-auto max-w-6xl rounded-lg px-4 md:px-6">
        <div className="flex items-center gap-3">
          <a href="/" className="flex items-center gap-3">
            <div>
              <p className="text-sm uppercase tracking-[0.2em] text-gray-400">
                Wolfstoke
              </p>
              <p className="text-xs text-gray-500">Desenvolvimento Personalizado</p>
            </div>
          </a>
        </div>
        <div className="flex-1" />
        <div className="hidden items-center gap-3 text-sm md:flex">
          <a href="/" className="text-gray-300 hover:text-accent">
            Início
          </a>
          <a href="/posts" className="text-gray-300 hover:text-accent">
            Posts
          </a>
          <a href="#solucoes" className="text-gray-300 hover:text-accent">
            Soluções
          </a>
          <a href="#processo" className="text-gray-300 hover:text-accent">
            Processo
          </a>
          <a href="#investimento" className="text-gray-300 hover:text-accent">
            Investimento
          </a>
          <a href="#faq" className="text-gray-300 hover:text-accent">
            FAQ
          </a>
          <a href="#contato" className="text-gray-300 hover:text-accent">
            Contato
          </a>
        </div>
      </div>
    </header>
  );
}
